...
const handleAddGoal = async (desc) => {
  if (!desc.trim()) {
    toast.warn("Goal description is required");
    return;
  }
  try {
...
const handleAddKRA = async (name, goalId) => {
  if (!name.trim() || !goalId) {
    toast.warn("KRA name and Goal are required");
    return;
  }
  try {
...
const handleAddKPI = async (name, kraId, goalId) => {
  if (!name.trim() || !kraId || !goalId) {
    toast.warn("KPI, Goal and KRA are required");
    return;
  }
  try {
...
const handleEditSave = async (type, item) => {
  try {
    if (type === "goal") {
      await axios.put(`${BASE_URL}/goal2/edit-goal2/${item.id}`, {
        goal_desc: item.goal_desc,
      });
    } else if (type === "kra") {
      await axios.put(`${BASE_URL}/kra2/edit-kra2/${item.id}`, {
        kra_name: item.kra_name,
        goalId: item.goalId,
      });
    } else if (type === "kpi") {
